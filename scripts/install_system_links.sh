...
# Define os caminhos
PROJECT_DIR="/mnt/disco_d/projetos/rust/cosmic-edit"
BINARY_PATH="$PROJECT_DIR/target/debug/cosmic-edit"
LOCAL_BIN_DIR="$HOME/.local/bin"
DESKTOP_DIR="$HOME/.local/share/applications"

echo "Configurando links do sistema para usar o cosmic-edit do projeto..."

# 1. Garante que o diretório bin local existe
mkdir -p "$LOCAL_BIN_DIR"

# 2. Cria um link simbólico para o binário do projeto
# Isso fará com que o comando 'cosmic-edit' no terminal use a versão do projeto
# (Assumindo que ~/.local/bin está no seu PATH)
ln -sf "$PROJECT_DIR/scripts/run.sh" "$LOCAL_BIN_DIR/cosmic-edit"
...
echo "Sucesso!"
echo "Terminal: Agora o comando 'cosmic-edit' usará a versão deste projeto."
